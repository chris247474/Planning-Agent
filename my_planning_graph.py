...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check for empty prenodes - taken from a tip in the forums
        if not node_a1.prenodes and not node_a2.prenodes:
            return False

        # if an effect of node_a1 is also in the negation set of node_a2, then return True
        for effect_a1_add in node_a1.action.effect_add:
            if effect_a1_add in node_a2.action.effect_rem:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check if each effect of 1st action node (add set) is in the negative precondition of the 2nd action node
        # return True (indicate mutex relationship) if so
        for effect_a1_add in node_a1.action.effect_add:
            if effect_a1_add in node_a2.action.precond_neg:
                return True

        # check if each effect of 2nd action node (add set) is in the negative precondition of the 1st action node
        # return True (indicate mutex relationship) if so
        for effect_a2_add in node_a2.action.effect_add:
            if effect_a2_add in node_a1.action.precond_neg:
                return True

        # check if each effect of 1st action node (rem set) is in the positive precondition of the 2nd action node
        # return True (indicate mutex relationship) if so
        for effect_a1_rem in node_a1.action.effect_rem:
            if effect_a1_rem in node_a2.action.precond_pos:
                return True

        # check if each effect of 2nd action node (rem set) is in the positive precondition of the 1st action node
        # return True (indicate mutex relationship) if so
        for effect_a2_rem in node_a2.action.effect_rem:
            if effect_a2_rem in node_a1.action.precond_pos:
                return True

        # if no bool condition was hit, then return no mutex relationship
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # for each parent pair in 1st and 2nd action node,
        # if no pair is mutex, then return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # if we're dealing w 2 node instances with the same symbol,
        # check if they're both positive or both negative
        # return the opopsite
        # in effect checking if node bool values are opposite of each other
        if node_s1.symbol == node_s2.symbol:
            return not (node_s1.is_pos == node_s2.is_pos)

        # return no mutex relationship otherwise
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # for each parent pair of the 2 nodes
        # check if they're pairwise mutually exclusive
        for node1parent in node_s1.parents:
            for node2parent in node_s2.parents:
                if not node1parent.is_mutex(node2parent) and not node2parent.is_mutex(node1parent) or\
                                node_s1 in node2parent.children or node_s2 in node1parent.children:
                    return False
        ...
        for goal in self.problem.goal:
            ...
            for s_level in self.s_levels:
                for state in s_level:
                    if goal == state.symbol and state not in visited and state.is_pos:
                        level_sum += levelcount
                        visited.add(state)
                levelcount += 1
